<template>
  <div class="container">
    ...
    <splitpanes
      class="main-masterdetail pading-left pading-right pading-bottom default-theme"
      horizontal
      style="height: 84vh; width: 97%"
    >
          ...
          <table class="data-table noselect">
            <thead class="data-table__thead">
              <tr>
                ...
                <th style="width:120px" class="number-right">Tổng nguyên giá</th>
                <th>Nội dung</th>
            ...
            <tbody class="data-table__tbody">
              <tr
                class="tr-tbody"
                v-for="(assetIncrement, index) in assetsIncrement"
                @dblclick="rowOnDblClick(assetIncrement)"
                :key="assetIncrement.voucherID"
              ...
              >
                <td>
                  <input
                    ...
                    name=""
                    id=""
                    class="ckb ckb-primary"
                    :value="assetIncrement.voucherID"
                            v-model="selected"
                  />
                </td>
                <td style="text-align: center">{{ index + 1 }}</td>
                <td>{{ assetIncrement.voucherCode }}</td>
                <td>{{ formatDate(assetIncrement.voucherDate)}}</td>
                <td>{{ formatDate(assetIncrement.incrementDate) }}</td>
                <td class="number-right">{{ formatPrice(assetIncrement.price) }}</td>
                <td class="text-hide-base">
                  <p>{{ assetIncrement.description }}</p>
                </td>
                <td class="table-option">
                  <div
                    class="table-eidt icon-pading icon-small"
                    @click="rowOnDblClick(assetIncrement)"
                  >
                    <span class="tooltip">Sửa (Ctrl + C)</span>
                  </div>
                  <div
                    class="table-replication icon-content icon-pading icon-small"
                  >
                    <span class="tooltip">Nhân bản </span>
                  </div>
                  <div
                    class="table-delete icon-small icon-pading"
                    @click="showDialogDeleteList(assetIncrement)"
                  >
        ...
        <div class="footer-table">
          <table>
            <tr class="summary-table">
              ...
              <td style="width:150px"></td>
              <td style="width:135px" class="number-right"><strong>{{formatPrice(100000000)}}</strong></td>
              <td></td>
              <td></td>
            </tr>
          </table>
          <div class="paging-table">
            ...
            <div class="table__paging--center">
              <div class="paging__button">
                ...
                ><span class="tooltip">Về trang đầu</span>
              </div>
              <div class="paging__button paging__button--prev icon-content">
                <span class="tooltip">Trang trước</span>
              </div>

              <div class="paging__button paging__button--next icon-content">
                <span class="tooltip">Trang sau</span>
              </div>
              <div class="paging__button">
                <i class="fa-solid fa-angles-right"></i
                ><span class="tooltip">Đến trang cuối</span>
              </div>
            </div>
          </div>
        </div>
      </pane>
      <pane style="z-index: 9; background: #fff;" size="38">
        <div class="table-detail">
          ...
          <TableBase
            ...
            :isShowPaging = "true"
            :voucherID="this.selected[0]"
            v-on:listFixedAssetId="getListFixedAssetId"
          />
        </div>
      </pane>
    </splitpanes>
    ...
    <div class="dialog__content">
      <div class="dialog__container">
        ...
        <div class="dialog__body">
          <i
            style="color: #eddc1f"
            class="fa-solid fa-3x fa-triangle-exclamation"
          ></i>
      ...
      <div class="dialog__footer">
        ...
        <button
          ...
          class="btn-dialog btn btn-cancel-dialog-asset warning"
        >
          Không
        </button>
      </div>
  ...
  <TheDialogMasterDetail
    ...
    :showDetailChil="isShowDialogDetail"
    :showDetailFunction="showDetail"
    :assetIncrementSelected="assetIncrementSelected"
    :title="title"
    :isFormAdd="isFormAdd"
    :isFormUpdate="isFormUpdate"
    :isFormDuplicate="isFormDuplicate"
    ...
    */
    ...
    btnDeleteOnClick() {
      try {
        ...
        axios
          .delete(
            `${EndPoint.END_POINT_FIXED_ASSET_INCREMENT}?recordID=${this.selected[0]}`
          )
          .then((res) => {
            if (res.status == 200) {
...
*/
    ...
    activeAssetNew(id) {
      try {
        (this.selected = []), this.selected.push(id);
        this.checkActive(id);
      } catch (error) {
        console.log(error);
      }
